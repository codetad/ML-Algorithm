...
int BST::Search(int x, Node** P, Node** D) //P는 직전에 본 node
{
	*D = root; *P = NULL;
	while (*D != NULL) {
		if ((*D)->a == x) { return 1; }
...
int BST::Delete(int x) 
{
	...
	if (Search(x, &P, &D) == 1) {
		if (D->L == NULL && D->R == NULL) {
			...
			Node* RLL, *RLLP;
			RLL = D->R; RLLP = D;
			while (RLL->L != NULL) { RLLP = RLL; RLL = RLL->L; }
			D->a = RLL->a;
...
int BST::Print(Node* C, int depth, int LR) // C is current Node, LR에 배열이 들어가면 세로줄 칠 수 있음
{
