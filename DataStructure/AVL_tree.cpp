...
int AVL::Search(int x, Node** S) {
	Node* D;
	D = root;
	while (D != NULL) {
		if (D->a == x) { *S = D; return 1; }
		else if (D->a < x) { D = D->R; }
		else { D = D->L; }
	}
	return 0;
...
int AVL::RInsert(int x, Node* D, Node** RP)
{
	...
	if (abs(D->LL - D->RL) > 1) {
		...
		LC = D->L; RC = D->R; //left child, right child
...
int main() {
	...
	while (1) {
		...
		if (com == 'q') { break; }
			...
			scanf("%d", &x);
			in = TT.Search(x, &D);
			if (in == 0) cout << "Not Found. \n\n" << endl;
			else cout << "Find at address " << D << "\n\n";
		}
		else if (com == 'i') {
			scanf("%d", &x);
			if (TT.Insert(x) == 1) cout << "Insert Successful." << "\n";
